...
# CHANGED THE ABOVE CODE TO COMPLY WITH SOLID PRINCIPLES

# BLOGSITE (with applying SOLID principles)
...
class User_auth
    ...
    def login_user()
        ...
        user = @data.users_data.find {|x| x["username"] == user_name}
        if user
            if BCrypt::Password.new(user["password"]) == user_password
                puts "\nlogin successful!"
...
class Data
    attr_accessor :users_data, :load_user_data, :save_user_data
    def initialize
        @users_data = []
        load_user_data()
    end
...
class User
    attr_accessor :data, :post_service
    def initialize()
        @post_service = Post_service.new()
        @data = Data.new()
    end
    ...
    def access_user(user)
        loop do
            ...
            case choice
            when 1
                @post_service.create_post(user)
            when 2
                @post_service.view_posts(user)
            when 3
                @post_service.edit_posts(user)
            when 4
                @post_service.delete_post(user)
            when 5
                edit_username(user)
            when 6
                edit_password(user)
